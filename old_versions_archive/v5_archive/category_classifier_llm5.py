"""
Product Category CLASSIFIER V5 - OPTIMIZED with system_instruction approach
...
- Output: tf_menu_labeled_v5.csv (Appends results incrementally)

VERSION 5 IMPROVEMENTS:
- Uses system_instruction (taxonomy NOT embedded in prompt) - 8-10x faster!
- Detailed logging at every step
- Optimal batch size (25 items) and workers (4)
- Proper error handling with exponential backoff
- Telegram minute reports with detailed progress

ERROR HANDLING:
- Critical (STOP APP): 403, GOAWAY, permission errors
...
OUTPUT_FILE = "tf_menu_labeled_v55_optimized.csv"
TAXONOMY_FILE = "category_definitions3.json"

CONFIG: Dict[str, Any] = {
    # --- Gemini ---
    "GEMINI_API_KEY": os.getenv("GENAI_API_KEY", "AIzaSyAGT6ChSwKaTQVagAeu4vD-RseFA6sSrL8"),
    "MODEL_NAME": "gemini-2.5-flash",  # Using latest flash model
    "TEMPERATURE": 0.0,

    # --- Processing --- OPTIMIZED for system_instruction approach
    "MAX_WORKERS": 4,   # optimal for throughput without overwhelming API
    "BATCH_SIZE": 25,   # much better than 5! (taxonomy not in prompt anymore)
...
def main():
    ...
    telegram.send(
        f"[tf_menu] Started V5 OPTIMIZED.\n"
        ...
        f"Approach: system_instruction (taxonomy NOT in prompt - 8-10x faster!).\n"
        f"Thread-local models for true parallel processing.\n"
